...
sp_input <-
  read.csv(file = "./dados/ocorrencias/pisp_input_setupsdmdata.csv", sep = ",") #
## Colocando o "_" entre genero e especie
sp_input$species <-
...
sp_input <-
  read.csv(file = "./dados/ocorrencias/cesp_input_setupsdmdata.csv", sep = ",")
...
lista_arquivos <-
  list.files(
    "./dados/raster/variaveis_cortadas_brasil/",
    full.names = T,
    pattern = ".tif"
  )
